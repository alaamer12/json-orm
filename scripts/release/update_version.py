...
def create_changelog(version: str, changes: Dict[str, str], quiet: bool = False, changelog_path: str = "changelog", root_dir: Optional[Path] = None) -> None:
    ...
    from scripts.release.changelog import create_changelog as create_changelog_file
...
def create_release_notes(new_version: str, changes: Dict[str, str], release_path: str) -> None:
    from scripts.release.simple_release import create_release_notes
...
def parse_arguments():
    ...
    parser.add_argument("-q", "--quiet", action="store_fastapi-utilites", help="Suppress output messages")
    parser.add_argument("-r", "--rollback", action="store_fastapi-utilites", help="Rollback to previous version")
...
def add_changelog_arguments(parser):
    ...
    changelog_group.add_argument("-sc", "--simple-changelog", action="store_fastapi-utilites", help="Create a simple changelog entry")
    changelog_group.add_argument("-cc", "--comprehensive-changelog", action="store_fastapi-utilites", help="Create a comprehensive changelog")
...
def add_release_arguments(parser):
    ...
    release_group.add_argument("-sr", "--simple-release", action="store_fastapi-utilites", help="Create simple release notes")
    release_group.add_argument("-cr", "--comprehensive-release", action="store_fastapi-utilites", help="Create comprehensive release notes")
...
def handle_changelog_creation_utility(args, new_version, path):
    ...
    if args.simple_changelog:
        from scripts.release.simple_changelog import create_changelog, get_changelog_entry
        entry = get_changelog_entry()
        create_changelog(new_version, entry, path)
    elif args.comprehensive_changelog:
        from scripts.release.changelog import create_changelog, get_changelog_entries
...
def handle_release_notes_creation(args, new_version, path):
    ...
    if args.simple_release:
        from scripts.release.simple_release import create_release_notes, get_release_notes
        notes = get_release_notes(new_version)
        create_release_notes(new_version, notes, path)
    elif args.comprehensive_release:
        from scripts.release.release import create_release_notes, get_release_notes
        notes = get_release_notes(new_version)
        create_release_notes(new_version, notes, path)
